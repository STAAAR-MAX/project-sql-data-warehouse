-- ==============================================================================
-- DDL Script: Create Silver Tables
-- ==============================================================================

-- Tujuan Script:
-- Script ini digunakan untuk membuat tabel-tabel pada schema 'silver'.
-- Jika tabel sudah ada sebelumnya, maka tabel tersebut akan dihapus (DROP)
-- dan dibuat ulang (CREATE) menggunakan struktur terbaru.
--
-- Script ini dijalankan untuk mendefinisikan ulang struktur DDL
-- dari tabel-tabel pada layer bronze ke layer silver.
--
-- Layer silver berfungsi sebagai penyimpanan data yang telah melalui
-- proses pembersihan dasar (cleansing), standarisasi, dan validasi awal
-- sebelum digunakan pada layer berikutnya (gold).
--
-- Sumber data pada layer silver berasal dari file CSV sistem CRM dan ERP
-- yang telah dimuat sebelumnya ke layer bronze.
--
-- File Script  : ddl_silver.sql
-- Schema Target: silver
-- Database     : Data Warehouse
-- ==============================================================================


IF OBJECT_ID ('silver.crm_cust_info', 'U') IS NOT NULL
	DROP TABLE silver.crm_cust_info;
CREATE TABLE silver.crm_cust_info (
	cst_id				INT,
	cst_key				NVARCHAR(50),
	cst_firstname		NVARCHAR(50),
	cst_lastname		NVARCHAR(50),
	cst_marital_status	NVARCHAR(50),
	cst_gndr			NVARCHAR(50),
	cst_create_date		DATE,
	dwh_create_time DATETIME2 DEFAULT GETDATE()
);

-- Membuat tabel pada database Data Warehouse di layer silver
-- yang bersumber dari file CSV CRM (prd_info)
IF OBJECT_ID ('silver.crm_prd_info', 'U') IS NOT NULL
	DROP TABLE silver.crm_prd_info;
CREATE TABLE silver.crm_prd_info (
	prd_id			INT,
	prd_key			NVARCHAR(50),
	prd_nm			NVARCHAR(50),
	prd_cost		INT,
	prd_line		NVARCHAR(50),
	prd_start_dt	DATETIME,
	prd_end_dt		DATETIME,
	dwh_create_time DATETIME2 DEFAULT GETDATE()
);

-- Membuat tabel pada database Data Warehouse di layer silver
-- yang bersumber dari file CSV CRM (sales_details)
IF OBJECT_ID ('silver.crm_sales_details', 'U') IS NOT NULL
	DROP TABLE silver.crm_sales_details;
CREATE TABLE silver.crm_sales_details (
	sls_ord_num		NVARCHAR(50),
	sls_prd_key		NVARCHAR(50),
	sls_cust_id		INT,
	sls_order_dt	INT,
	sls_ship_dt		INT,
	sls_due_dt		INT,
	sls_sales		INT,
	sls_quantity	INT,
	sls_price		INT,
	dwh_create_time DATETIME2 DEFAULT GETDATE()
);

-- Membuat tabel pada database Data Warehouse di layer silver
-- yang bersumber dari file CSV ERP (CUST_AZ12)
IF OBJECT_ID ('silver.erp_cust_az12', 'U') IS NOT NULL
	DROP TABLE silver.erp_cust_az12;
CREATE TABLE silver.erp_cust_az12(
	cid		NVARCHAR(50),
	bdate	DATE,
	gen		NVARCHAR(50),
	dwh_create_time DATETIME2 DEFAULT GETDATE()
);

-- Membuat tabel pada database Data Warehouse di layer silver
-- yang bersumber dari file CSV ERP (LOC_AZ12)
IF OBJECT_ID ('silver.erp_loc_a101', 'U') IS NOT NULL
	DROP TABLE silver.erp_loc_a101;
CREATE TABLE silver.erp_loc_a101(
	cid		NVARCHAR(50),
	cntry	NVARCHAR(50),
	dwh_create_time DATETIME2 DEFAULT GETDATE()
);

-- Membuat tabel pada database Data Warehouse di layer silver
-- yang bersumber dari file CSV ERP (PX_CAT_G1V2)
IF OBJECT_ID ('silver.erp_px_cat_g1v2', 'U') IS NOT NULL
	DROP TABLE silver.erp_px_cat_g1v2;
CREATE TABLE silver.erp_px_cat_g1v2(
	id			NVARCHAR(50),
	cat			NVARCHAR(50),
	subcat		NVARCHAR(50),
	maintenance NVARCHAR(50),
	dwh_create_time DATETIME2 DEFAULT GETDATE()
);


